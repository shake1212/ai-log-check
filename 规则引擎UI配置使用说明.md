...
### 1. 数据库层
- 完成 添加 `enable_rule_engine` 字段（是否启用规则引擎）
- 完成 添加 `rule_engine_timeout` 字段（规则引擎超时时间）
- 完成 创建数据库迁移脚本

### 2. Java后端
- 完成 更新 `LogCollectorConfig` 实体类
- 完成 支持规则引擎配置的读取和保存

### 3. 前端UI
- 完成 更新 `LogCollectorService` TypeScript接口
- 完成 在配置模态框中添加规则引擎开关
- 完成 添加规则引擎超时配置
