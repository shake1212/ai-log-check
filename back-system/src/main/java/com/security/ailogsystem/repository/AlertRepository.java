...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    // 新增的统计查询方法
    long countByStatusAndTimestampBetween(Alert.AlertStatus status, LocalDateTime start, LocalDateTime end);
    
    long countByTimestampBetween(LocalDateTime start, LocalDateTime end);
    
    @Query("SELECT AVG(a.aiConfidence) FROM Alert a WHERE a.aiConfidence IS NOT NULL")
