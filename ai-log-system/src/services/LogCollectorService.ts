...
class LogCollectorService {
  ...
  async startCollector(collectorId: string): Promise<boolean> {
    ...
    if (!response.ok) {
      throw new Error(`启动采集器失败 (${response.status})`);
    }
    return true;
  }
  ...
  async stopCollector(collectorId: string): Promise<boolean> {
    ...
    if (!response.ok) {
      throw new Error(`停止采集器失败 (${response.status})`);
    }
    return true;
  }
  ...
  async resolveAlert(alertId: string): Promise<boolean> {
    ...
    if (!response.ok) {
      throw new Error(`解决告警失败 (${response.status})`);
    }
    return true;
  }
  ...
  async updateConfig(config: LogCollectorConfig): Promise<boolean> {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => '');
      throw new Error(`更新配置失败 (${response.status}): ${text || response.statusText}`);
    }
    return true;
  }
